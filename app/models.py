...
class RetentionPolicyCreate(BaseModel):
    ...
    retention_days: StrictInt = Field(
        ge=0, description="Non-negative number of days a snapshot must age before deletion"
    )
...
class SnapshotDeletionRequestCreate(BaseModel):
    ...
    reason: StrictStr = Field(description="Non-empty reason for requesting the deletion")
...
class SnapshotDeletionConfirmation(SnapshotDeletionRequest):
    confirmed_at: str
