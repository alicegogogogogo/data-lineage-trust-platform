"""Tests for the read-only lineage source shortest-path query."""
...
def test_direct_source_has_path_length_one(client: TestClient) -> None:
    setup_branching_graph(client)
    body = sources(client, "mid_b", 1, "b1")
...
def test_multi_hop_paths_and_diamond_tie_break(client: TestClient) -> None:
    setup_branching_graph(client)
    body = sources(client, "mart_c", 1, "c1")
    ...
    a1 = body["origins"][2]
    # a1 reaches c1 via two length-2 paths (through b1 and through b2); the
    # lexicographically smallest node sequence goes via b1.
...
def test_shortest_path_beats_longer_route(client: TestClient) -> None:
    # s.f -> a.f -> b.f -> d.f and a separate direct s.f -> d.f edge; from
    # d.f looking upstream the direct edge wins regardless of insertion order.
    ...
    origins = sources(client, "d", 1, "f")["origins"]
    s_origin = next(item for item in origins if item["dataset"] == "s")
...
def test_equal_length_tie_break_by_first_diverging_node(
    ...
    # s.f -> a.w -> d.z and s.f -> a.x -> d.z: from d.z looking upstream,
    # s.f is reached via two equal-length routes; the route via field "w" is
    # lexicographically smaller than via "x".
    make_dataset(client, "s", ["f"])
    ...
    origins = sources(client, "d", 1, "z")["origins"]
    s_origin = next(item for item in origins if item["dataset"] == "s")
...
def test_without_upstream_sources_is_empty(client: TestClient) -> None:
    setup_branching_graph(client)
    body = sources(client, "raw_a", 1, "a1")
    assert body == {
    ...
    assert sources(client, "mart_c", 1, "c1")["origins"]
    assert sources(client, "raw_a", 1, "a2")["origins"] == []
...
def test_cycle_terminates_with_shortest_paths(client: TestClient) -> None:
    ...
    body = sources(client, "cyc_x", 1, "f")
    assert body["source"] == ref("cyc_x", 1, "f")
    # Walking upstream from cyc_x: cyc_z feeds it directly, cyc_y feeds cyc_z;
    # the start field itself never appears as an origin.
    assert [
...
def test_origins_sorted_by_dataset_version_field(client: TestClient) -> None:
    ...
    add_link(client, ("b_ds", 1, "f"), ("z_dst", 1, "t"))
    add_link(client, ("a_ds", 1, "f2"), ("z_dst", 1, "t"))
    add_link(client, ("a_ds", 1, "f1"), ("z_dst", 1, "t"))
    add_link(client, ("a_ds", 2, "f1"), ("z_dst", 1, "t"))

    body = sources(client, "z_dst", 1, "t")
    assert [
        (item["dataset"], item["version"], item["field"])
        for item in body["origins"]
    ] == [
...
def test_field_parameter_is_matched_literally(client: TestClient) -> None:
    setup_branching_graph(client)
    # No trimming: a whitespace-padded name names no field and is a 404.
    response = client.get(
...
def test_empty_origins_document_also_has_fixed_shape(client: TestClient) -> None:
    ...
    assert response.content == (
        ...
        b'"origins":[],"direct_count":0,"indirect_count":0,'
        b'"source_dataset_count":0}\n'
    )
...
def test_unknown_dataset_returns_404(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/ghost/versions/1{SOURCE_PATHS}", params={"field": "c1"}
    )
    assert response.status_code == 404
    body = response.json()
...
def test_unknown_version_returns_404(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/mart_c/versions/99{SOURCE_PATHS}", params={"field": "c1"}
    )
    assert response.status_code == 404
    assert response.json()["error"] == "not_found"
...
def test_unknown_field_returns_404(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/mart_c/versions/1{SOURCE_PATHS}",
...
def test_missing_field_param_returns_422(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(f"/datasets/mart_c/versions/1{SOURCE_PATHS}")
...
def test_404_takes_precedence_over_422(client: TestClient) -> None:
    ...
    response = client.get(
        f"/datasets/mart_c/versions/99{SOURCE_PATHS}",
        params={"field": "c1", "bogus": "x"},
    )
    assert response.status_code == 404
...
def test_only_get_is_accepted(client: TestClient) -> None:
    setup_branching_graph(client)
    url = f"/datasets/mart_c/versions/1{SOURCE_PATHS}"
...
def test_query_is_read_only_and_never_uses_cache(
    ...
    assert sources(client, "mart_c", 1, "c1") == body
...
def test_rejected_requests_write_nothing(
    ...
    client.get(f"/datasets/mart_c/versions/1{SOURCE_PATHS}")
    client.get(
        f"/datasets/mart_c/versions/1{SOURCE_PATHS}",
        params={"field": "c1", "bogus": "x"},
    )
