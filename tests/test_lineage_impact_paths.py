"""Tests for the read-only lineage impact shortest-path query."""
...
IMPACT_PATHS = "/lineage/impact-paths"
...
def paths(
    ...
    response = client.get(
        f"/datasets/{dataset}/versions/{version}{IMPACT_PATHS}",
        params={"field": field},
    )
...
def test_direct_downstream_has_path_length_one(client: TestClient) -> None:
    setup_branching_graph(client)
    body = paths(client, "mid_b", 1, "b1")
...
def test_multi_hop_paths_and_diamond_tie_break(client: TestClient) -> None:
    setup_branching_graph(client)
    body = paths(client, "raw_a", 1, "a1")

    # Impacts are sorted by location key (dataset, version, field): mart_c
    # before mid_b; b1 before b2.
    assert [
    ...
    c1 = body["impacts"][0]
    # c1 has two length-2 paths (via b1 and via b2); the lexicographically
    # smallest node sequence goes via b1.
    assert c1["path_length"] == 2
    assert c1["path"] == [
        ref("raw_a", 1, "a1"),
        ref("mid_b", 1, "b1"),
        ref("mart_c", 1, "c1"),
    ...
    assert body["impacts"][1]["path"] == [
        ref("raw_a", 1, "a1"),
        ref("mid_b", 1, "b1"),
    ]
    assert body["impacts"][2]["path"] == [
        ref("raw_a", 1, "a1"),
        ref("mid_b", 1, "b2"),
    ]
...
def test_shortest_path_beats_longer_route(client: TestClient) -> None:
    # s -> a.f -> b.f -> d.f and a separate direct s -> d.f edge; the direct
    # edge wins regardless of insertion order.
    ...
    impacts = paths(client, "s", 1, "f")["impacts"]
    d_impact = next(item for item in impacts if item["dataset"] == "d")
    assert d_impact["path_length"] == 1
    assert d_impact["path"] == [ref("s", 1, "f"), ref("d", 1, "f")]
...
def test_equal_length_tie_break_by_first_diverging_node(
    ...
    impacts = paths(client, "s", 1, "f")["impacts"]
    d_impact = next(item for item in impacts if item["dataset"] == "d")
    assert d_impact["path"] == [
        ref("s", 1, "f"),
        ref("a", 1, "w"),
        ref("d", 1, "z"),
    ]
    assert d_impact["path_length"] == 2


def test_without_downstream_impacts_is_empty(client: TestClient) -> None:
    setup_branching_graph(client)
    body = paths(client, "mart_c", 1, "c1")
    assert body == {
        "source": ref("mart_c", 1, "c1"),
        "impacts": [],
        "direct_count": 0,
        "indirect_count": 0,
    }
    assert paths(client, "raw_a", 1, "a2")["impacts"] == []
...
def test_cycle_terminates_with_shortest_paths(client: TestClient) -> None:
    ...
    body = paths(client, "cyc_x", 1, "f")
    assert body["source"] == ref("cyc_x", 1, "f")
    assert [
        (item["dataset"], item["field"]) for item in body["impacts"]
    ] == [("cyc_y", "f"), ("cyc_z", "f")]
    y_item = body["impacts"][0]
    z_item = body["impacts"][1]
    assert y_item["path"] == [ref("cyc_x", 1, "f"), ref("cyc_y", 1, "f")]
    assert y_item["path_length"] == 1
    assert z_item["path"] == [
        ref("cyc_x", 1, "f"),
        ref("cyc_y", 1, "f"),
        ref("cyc_z", 1, "f"),
    ]
    assert z_item["path_length"] == 2
    assert body["direct_count"] == 1
    assert body["indirect_count"] == 1


def test_paths_sorted_by_dataset_version_field(client: TestClient) -> None:
...
def test_field_parameter_is_trimmed(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/mid_b/versions/1{IMPACT_PATHS}",
        params={"field": "  b1  "},
    )
    assert response.status_code == 200, response.text
    assert response.json()["source"] == ref("mid_b", 1, "b1")
...
def test_document_is_compact_with_fixed_key_order_and_newline(
    ...
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/raw_a/versions/1{IMPACT_PATHS}", params={"field": "a1"}
    )
    expected = (
        b'{"source":{"dataset":"raw_a","version":1,"field":"a1"},'
        b'"impacts":['
        b'{"dataset":"mart_c","version":1,"field":"c1",'
        b'"path":[{"dataset":"raw_a","version":1,"field":"a1"},'
        ...
        b'{"dataset":"mid_b","version":1,"field":"b2"}],'
        b'"path_length":1}],'
        b'"direct_count":2,"indirect_count":1}\n'
    )
...
def test_empty_impacts_document_also_has_fixed_shape(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/mart_c/versions/1{IMPACT_PATHS}", params={"field": "c1"}
    )
    assert response.content == (
        b'{"source":{"dataset":"mart_c","version":1,"field":"c1"},'
        b'"impacts":[],"direct_count":0,"indirect_count":0}\n'
    )
...
def test_unknown_dataset_returns_404(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/ghost/versions/1{IMPACT_PATHS}", params={"field": "a1"}
    )
...
def test_unknown_version_returns_404(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/raw_a/versions/99{IMPACT_PATHS}", params={"field": "a1"}
    )
    assert response.status_code == 404
...
def test_unknown_field_returns_404(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(
        f"/datasets/raw_a/versions/1{IMPACT_PATHS}",
...
def test_missing_field_param_returns_422(client: TestClient) -> None:
    setup_branching_graph(client)
    response = client.get(f"/datasets/raw_a/versions/1{IMPACT_PATHS}")
...
def test_404_takes_precedence_over_422(client: TestClient) -> None:
    ...
    response = client.get(f"/datasets/ghost/versions/1{IMPACT_PATHS}")
    assert response.status_code == 404

    # Unknown dataset beats an extra query parameter and a request body.
    response = client.request(
        "GET",
        f"/datasets/ghost/versions/1{IMPACT_PATHS}",
        params={"field": "a1", "bogus": "x"},
        content=b"{}",
    )
    assert response.status_code == 404
    ...
    response = client.get(
        f"/datasets/raw_a/versions/99{IMPACT_PATHS}",
        params={"field": "a1", "bogus": "x"},
    )
    assert response.status_code == 404
...
def test_only_get_is_accepted(client: TestClient) -> None:
    setup_branching_graph(client)
    url = f"/datasets/raw_a/versions/1{IMPACT_PATHS}"
...
def test_query_is_read_only_and_never_uses_cache(
    ...
    assert paths(client, "raw_a", 1, "a1") == body
...
def test_rejected_requests_write_nothing(
    ...
    client.get(f"/datasets/raw_a/versions/1{IMPACT_PATHS}")
    client.get(
        f"/datasets/raw_a/versions/1{IMPACT_PATHS}",
        params={"field": "a1", "bogus": "x"},
    )
...
def test_existing_impact_endpoint_is_unaffected(client: TestClient) -> None:
    setup_branching_graph(client)
    body = paths(client, "raw_a", 1, "a1")
    assert [
        (item["dataset"], item["version"], item["field"])
...
response = client.get(
    "/datasets/src/versions/1/lineage/impact-paths",
    params={"field": "s1"},
)
assert response.status_code == 200, response.text
...
def test_paths_are_identical_after_restart(tmp_path: Path) -> None:
    ...
    assert document["impacts"][0]["path_length"] == 2
